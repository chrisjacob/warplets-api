...
export function resolveAppSlugFromUrl(url: URL): AppSlug {
  ...
  if ((STONKLETS_APP_HOSTS as readonly string[]).includes(hostname)) return STONKLETS_APP_SLUG;
  ...
  if (cleanPath === STONKLETS_APP_PATH || cleanPath.startsWith(`${STONKLETS_APP_PATH}/`)) return STONKLETS_APP_SLUG;
...
export function resolveAppSlugFromAppFid(
  ...
  if (mapping.stonklets != null && appFid === mapping.stonklets) return STONKLETS_APP_SLUG;
