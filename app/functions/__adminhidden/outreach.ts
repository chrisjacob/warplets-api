    ...
    <section class="panel">
      <div class="table-wrap">
        <table>
          ...
          <tbody id="rowsBody"><tr><td colspan="7" class="muted empty-row" style="text-align:center;padding:24px">Loading…</td></tr></tbody>
        </table>
      </div>
...
<script>
  ...
  function renderRows() {
    ...
    if (!rows.length) {
      body.innerHTML = '<tr><td colspan="7" class="muted empty-row" style="text-align:center;padding:24px">No matching active holders. Run a refresh if activity has not been scanned recently.</td></tr>';
    } else {
      body.innerHTML = rows.map(function(row, index) {
        ...
        return '<tr>' +
          '<td data-label="Latest cast"><strong>' + esc(formatTime(row.castAt)) + '</strong><div class="cast-meta">' + (row.isReply ? 'Reply' : 'Cast') + '</div></td>' +
          '<td data-label="Holder"><div class="identity">' + avatar + '<div><div class="username">@' + esc(row.username) + '</div><div>' + esc(row.displayName || '') + '</div><div class="cast-meta">FID ' + esc(row.fid) + '</div></div></div></td>' +
          '<td data-label="Cast"><div class="cast">' + esc(row.castText) + '</div><div class="cast-meta"><a href="' + esc(castUrl) + '" target="_blank" rel="noreferrer">View cast</a></div></td>' +
          '<td data-label="Warplet"><a href="https://warplet.10x.meme/?search=' + esc(row.tokenId) + '&amp;warplet=' + esc(row.tokenId) + '" target="_blank" rel="noreferrer">#' + esc(row.tokenId) + '</a><div class="counts">' + esc(row.ownedCount) + ' owned</div></td>' +
          '<td data-label="Converted"><span class="pill ' + (row.converted ? 'yes' : 'no') + '">' + (row.converted ? 'Yes' : 'No') + '</span>' + (row.optedOut ? ' <span class="pill no">Opted out</span>' : '') + '<div class="counts">' + esc(formatTime(row.convertedAt)) + '</div></td>' +
          '<td data-label="Outreach"><strong>' + esc(row.outreachCount) + '</strong><div class="counts">FC ' + esc(row.farcasterOutreachCount) + ' · X ' + esc(row.xOutreachCount) + '<br>' + esc(formatTime(row.lastOutreachAt)) + '</div></td>' +
          '<td data-label="Actions"><div class="actions">' + (row.optedOut ? '<span class="muted">Outreach disabled</span>' : '<button data-action="reply" data-index="' + index + '">Reply on Farcaster</button>' +
