...
export const onRequestPost: PagesFunction<WarpmojiAdminEnv> = async ({ request, env }) => {
  ...
  await env.WARPLETS.batch([
    env.WARPLETS.prepare("UPDATE warpmoji_candidates SET status = 'approved', reviewed_at = CURRENT_TIMESTAMP, reviewed_by_fid = ?, updated_at = CURRENT_TIMESTAMP WHERE canonical_emoji = ? AND status != 'rejected'").bind(admin.fid, emoji),
    env.WARPLETS.prepare("UPDATE warpmoji_emoji_groups SET reviewed_at = CURRENT_TIMESTAMP, reviewed_by_fid = ?, approved_count = ?, updated_at = CURRENT_TIMESTAMP WHERE canonical_emoji = ?").bind(admin.fid, count.count, emoji),
  ]);
  await auditWarpmoji(env.WARPLETS, admin.fid, "group.review", emoji, { approved: count.count });
