...
export const onRequestGet: PagesFunction<Env> = async (context) => {
  ...
  const appSlug = resolveAppSlugFromUrl(requestUrl);
  const appUrl = resolveBaseNotificationConfig(context.env, appSlug).appUrl;
  const forceRefresh = requestUrl.searchParams.get("refresh") === "1";
  const cached = await context.env.WARPLETS.prepare(
    `SELECT app_pinned, notifications_enabled, checked_at
     FROM base_notification_status_cache
     WHERE wallet_address = ? AND app_url = ?
     LIMIT 1`,
  ).bind(session.walletAddress, appUrl).first<{ app_pinned: number; notifications_enabled: number; checked_at: string }>();
  ...
  try {
    const status = await getBaseNotificationStatus(context.env, session.walletAddress, appSlug);
    ...
    ).bind(session.walletAddress, appUrl, status.appPinned ? 1 : 0, status.notificationsEnabled ? 1 : 0, now, JSON.stringify(status)).run();
