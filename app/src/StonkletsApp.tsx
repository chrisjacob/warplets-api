...
import { loadAppSession, logoutAppPrincipal, verifyFarcasterQuickAuth, type AppSessionState } from "./appSession";
...
import { filterAndSortStonklets, stonkletMetric, type StonkletsMarketEntry as MarketEntry } from "./stonkletsMarket";
...
function StonkletsHeaderAccount({ session, simplifiedFarcaster, open, centered, onOpenChange, onConnect, onShowFavourites, onEnableNotifications, onDisconnect }: {
  ...
  return <div className="search-header-account" ref={rootRef}>
    {!connected ? <button type="button" className="search-header-connect-button" onClick={onConnect}>Connect</button> : <button type="button" className="search-header-avatar-button" aria-haspopup="menu" aria-expanded={open} aria-label="Open account menu" onClick={() => onOpenChange(!open)}>
      <span className="search-header-avatar-stack">
        ...
        {session?.farcasterFid && <span className="search-header-avatar-frame search-header-avatar-frame--identity"><img src={session.farcasterProfile?.pfpUrl ?? "/farcaster.webp"} alt="Farcaster" className="search-header-avatar-image" /></span>}
        {simplifiedFarcaster && !session?.farcasterFid && <span className="search-header-avatar-frame search-header-avatar-frame--identity"><img src="/farcaster.webp" alt="Farcaster" className="search-header-avatar-image" /></span>}
      </span>
    </button>}
    {open && <div className={`search-header-account-menu${centered ? " search-header-account-menu--centered" : ""}`} role="menu">
      ...
      <button type="button" role="menuitem" className="search-header-account-menu__connection" onClick={() => simplifiedFarcaster ? onOpenChange(false) : run(onConnect)}><span className="search-header-account-menu__avatar-frame"><img src={session?.farcasterProfile?.pfpUrl ?? "/farcaster.webp"} alt="" /></span><span>{session?.farcasterProfile?.username ? `@${session.farcasterProfile.username}` : simplifiedFarcaster ? "Farcaster identity" : "Connect social"}</span></button>
...
function DeferredChart({ pairId, asset, range, periodChange, fallbackImage }: { pairId: string; asset: MarketSide; range: StonkletChangeRange; periodChange: number | null; fallbackImage: string }) {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      fetch(`/api/stonklets/chart?pair=${encodeURIComponent(pairId)}&asset=${asset}&range=${range}`, { credentials: "same-origin" }).then(async (response) => {
        if (!response.ok) throw new Error("Chart unavailable");
        return response.json() as Promise<{ points?: { time: number; value: number; price: number }[] }>;
      }),
    ]).then(([charts, payload]) => {
      ...
      const line = chart.addSeries(charts.LineSeries, {
        color: (periodChange ?? 0) >= 0 ? "#00ff66" : "#ff496c",
        lineWidth: 2,
  ...
  }, [asset, near, pairId, periodChange, range]);
  const rangeLabel = STONKLET_CHANGE_RANGE_LABELS[range];
  return <div className="stonklets-chart" role="img" aria-label={status === "ready" ? `Normalized ${rangeLabel} percentage chart. Start price ${endpointPrices ? priceText(endpointPrices.start) : "unavailable"}. End price ${endpointPrices ? priceText(endpointPrices.end) : "unavailable"}. Change ${changeText(periodChange)}` : status === "idle" || status === "loading" ? `Loading ${rangeLabel} market chart` : "Stonklet artwork shown while market chart data is unavailable"}>
    ...
    {status === "ready" && periodChange != null && <strong className={periodChange >= 0 ? "is-positive" : "is-negative"}>{changeText(periodChange)}</strong>}
    {status === "ready" && endpointPrices && <div className={`stonklets-chart-price-range ${(periodChange ?? 0) >= 0 ? "is-positive-range" : "is-negative-range"}`} aria-hidden="true"><span>{priceText(endpointPrices.start)}</span><b>➜</b><span>{priceText(endpointPrices.end)}</span></div>}
  </div>;
}

function AssetCard({ entry, asset, range, favourite, busy, onFavourite, onTrade }: { entry: MarketEntry; asset: MarketSide; range: StonkletChangeRange; favourite: boolean; busy: boolean; onFavourite: () => void; onTrade: () => void }) {
  ...
  const ctaLabel = asset === "stonklet" ? `Vote for $${entry.stonklet.symbol}/${entry.stock.symbol}` : `Trade $${entry.stock.symbol}`;
  return <article className="stonklets-asset-card">
    ...
    {asset === "stonklet" && entry.launchStatus !== "launched"
      ...
      : <DeferredChart pairId={entry.id} asset={asset} range={range} periodChange={periodChange} fallbackImage={highResolutionStonkletImage(entry.stonklet.image)} />}
    {asset === "stonklet" && entry.launchStatus !== "launched"
      ? <div className="stonklets-launch-panel" role="status">Vote to launch this Stonklet!</div>
    ...
    <button type="button" className="stonklets-trade stonklets-chart-cta" disabled={asset === "stonklet" && busy} onClick={asset === "stonklet" ? onFavourite : onTrade}>{ctaLabel}</button>
  </article>;
}

function CompactRow({ entry, asset, range, favourite, busy, onFavourite, onTrade }: { entry: MarketEntry; asset: MarketSide; range: StonkletChangeRange; favourite: boolean; busy: boolean; onFavourite: () => void; onTrade: () => void }) {
  const identity = asset === "stock" ? entry.stock : entry.stonklet;
  const metrics = asset === "stock" ? entry.stockMetrics : entry.stonkletMetrics;
  const periodChange = asset === "stock" ? entry.stockPeriodChange : entry.stonkletPeriodChange;
  const ctaLabel = asset === "stonklet" ? `Vote for $${entry.stonklet.symbol}/${entry.stock.symbol}` : `Trade $${entry.stock.symbol}`;
  ...
  return <div className={`stonklets-compact-row stonklets-compact-row--${asset}`}>
    ...
    {unlaunchedStonklet
      ? <div className="stonklets-grid-launch-panel" role="status">Vote to launch this Stonklet!</div>
    ...
    <button type="button" className="stonklets-trade stonklets-grid-cta" disabled={asset === "stonklet" && busy} onClick={asset === "stonklet" ? onFavourite : onTrade} title={ctaLabel} aria-label={ctaLabel}>{asset === "stonklet" ? "Vote" : "Trade"}</button>
  </div>;
}
...
export default function StonkletsApp() {
  ...
  const [entries, setEntries] = useState<MarketEntry[]>(defaultEntries);
  ...
  useEffect(() => {
    ...
    setLoading(true);
    fetch(`/api/stonklets/market?range=${changeRange}`, { signal: controller.signal }).then(async (response) => {
  ...
  }, [changeRange, direction, favouritesOnly, forceBstocksNotice, layout, market, order, page, search]);
  ...
  const selectedMarketFavourites = market === "stock" ? stockFavourites : favourites;
  ...
  const toggleFavourite = async (entry: MarketEntry, asset: MarketSide) => {
    if (!session?.authenticated || !favouriteIdentityReady) { setConnectOpen(true); return; }
    const next = !(asset === "stock" ? stockFavourites : favourites).has(entry.id);
    const busyKey = `${entry.id}:${asset}`;
    setBusyFavourite(busyKey);
    try {
      const response = await fetch("/api/stonklet-favourites", { method: "PUT", credentials: "same-origin", headers: { "content-type": "application/json" }, body: JSON.stringify({ stonkletId: entry.id, asset, favourited: next, notifyOnLaunch: asset === "stonklet" && next }) });
      ...
      if (asset === "stonklet" && next && (typeof Notification === "undefined" || Notification.permission !== "granted")) setLaunchPrompt(true);
    } catch (error) { setToast(error instanceof Error ? error.message : "Favourite could not be saved"); }
  ...
  const availableEntries = filtered.filter((entry) => entry.pairingStatus === "available");
  const upcomingEntries = filtered.filter((entry) => entry.pairingStatus === "upcoming");
  ...
  const renderPairs = (items: MarketEntry[]) => items.map((entry) => {
    ...
    return <article className={`stonklets-pair${layout === "single-grid" ? " stonklets-pair--single-grid" : ""}${layout === "single-chart" ? " stonklets-pair--single-chart" : ""}`} key={entry.id}>
      {isGridLayout(layout)
        ? assets.map((asset) => <CompactRow key={asset} entry={entry} asset={asset} range={changeRange} favourite={isFavourite(asset)} busy={isBusy(asset)} onFavourite={() => void toggleFavourite(entry, asset)} onTrade={() => goPage("trade", entry, asset)} />)
        : <div className={`stonklets-chart-pair${layout === "single-chart" ? " stonklets-chart-pair--single" : ""}`}>{assets.map((asset) => <AssetCard key={asset} entry={entry} asset={asset} range={changeRange} favourite={isFavourite(asset)} busy={isBusy(asset)} onFavourite={() => void toggleFavourite(entry, asset)} onTrade={() => goPage("trade", entry, asset)} />)}</div>}
    </article>;
  });
  const header = <MiniAppHeader
    ...
    rightAccessory={<StonkletsHeaderAccount
      ...
      onShowFavourites={() => { goPage("market"); setFavouritesOnly(true); }}
      onEnableNotifications={() => void subscribeToWebPush(["launches"]).then(() => setToast("Browser launch alerts enabled.")).catch((error) => setToast(error instanceof Error ? error.message : "Browser alerts could not be enabled"))}
  ...
  return <MiniAppShell>{bstocksNoticeOpen
    ...
    <main className="stonklets-main">
      <nav className="stonklets-nav" aria-label="Stonklets pages">{(["market", "trade", "portfolio", "leaderboard", "stats"] as Page[]).map((item) => <button type="button" key={item} className={page === item ? "is-active" : ""} onClick={() => goPage(item)}>{item[0]?.toUpperCase()}{item.slice(1)}</button>)}</nav>
      {page !== "market" ? <Placeholder page={page} pair={selectedPair} asset={selectedAsset} /> : <div className="stonklets-market-page">
        <section className="stonklets-search-row">
          <div className="relative flex min-w-0 flex-1">
            ...
            <div className="absolute bottom-1 right-1 top-1 flex items-center gap-1">
              ...
              <button type="button" aria-label={favouritesOnly ? "Remove favourite filter" : "Filter by my favourites"} title={favouritesOnly ? "Remove favourite filter" : "Filter by my favourites"} onClick={() => setFavouritesOnly((value) => !value)} className="group/fav -ml-0.5 mr-1.5 inline-flex h-full w-9 shrink-0 cursor-pointer items-center justify-center border-0 bg-transparent text-[#00FF00] transition-colors hover:text-[#8bbf8b]" aria-pressed={favouritesOnly}><SearchHeartIcon filled={favouritesOnly} /></button>
            </div>
          </div>
        </section>
        ...
        {isGridLayout(layout) && availableEntries.length > 0 && <div className="stonklets-compact-head"><span aria-hidden="true" /><span>MCap</span><span>24h Vol</span><span>Hold</span><span>{STONKLET_CHANGE_RANGE_LABELS[changeRange].toUpperCase()}</span></div>}
        <section className={`stonklets-results${isGridLayout(layout) ? " stonklets-results--grid" : ""}${layout === "single-grid" ? " stonklets-results--single-grid" : ""}`} aria-label="Available Stonklet pairings">{renderPairs(availableEntries)}</section>
        <button type="button" className="stonklets-upcoming-toggle" onClick={() => setShowUpcoming((value) => !value)} aria-expanded={showUpcoming}>{showUpcoming ? "Hide Upcoming Tokens" : "Show Upcoming Tokens"}</button>
        {showUpcoming && <section className={`stonklets-results stonklets-results--upcoming${isGridLayout(layout) ? " stonklets-results--grid" : ""}${layout === "single-grid" ? " stonklets-results--single-grid" : ""}`} aria-label="Upcoming Stonklet pairings">{isGridLayout(layout) && upcomingEntries.length > 0 && <div className="stonklets-compact-head"><span aria-hidden="true" /><span>MCap</span><span>24h Vol</span><span>Hold</span><span>{STONKLET_CHANGE_RANGE_LABELS[changeRange].toUpperCase()}</span></div>}{renderPairs(upcomingEntries)}</section>}
        ...
        {!showUpcoming && availableEntries.length === 0 && upcomingEntries.length > 0 && <p className="stonklets-empty-status">Matching results are in Upcoming Tokens.</p>}
    ...
    {launchPrompt && <div className="stonklets-alert-modal" role="dialog" aria-modal="true" aria-labelledby="stonklets-alert-title"><div><span className="stonklets-kicker">LAUNCH ALERTS</span><h2 id="stonklets-alert-title">Don’t miss the launch.</h2><p>Your vote saved the token-specific alert preference. Enable browser notifications too, or enable Farcaster notifications when adding this Mini App.</p><button type="button" onClick={() => { void subscribeToWebPush(["launches"]).then(() => { setToast("Browser launch alerts enabled."); setLaunchPrompt(false); }).catch((error) => setToast(error instanceof Error ? error.message : "Browser alerts could not be enabled")); }}>Enable browser alerts</button><button type="button" className="is-secondary" onClick={() => setLaunchPrompt(false)}>Maybe later</button></div></div>}
    {toast && <button type="button" className="stonklets-toast" onClick={() => setToast(null)}>{toast}</button>}</>}
  </MiniAppShell>;
}
