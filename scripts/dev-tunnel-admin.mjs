...
const PUBLIC_URL = "https://admin-local.10x.meme";
...
function spawnCloudflared(port) {
  ...
  const command = process.platform === "win32"
    ? `"${executable}" tunnel run --url http://127.0.0.1:${port} admin-local`
    : executable;
  const args = process.platform === "win32"
    ? []
    : ["tunnel", "run", "--url", `http://127.0.0.1:${port}`, "admin-local"];
  ...
  while (Date.now() < deadline) {
    try {
      const res = await fetch(`http://127.0.0.1:${port}/__adminhidden/`, {
        headers: { host: "admin-local.10x.meme" },
      });
...
async function main() {
  ...
  console.log(`Stable admin URL: ${PUBLIC_URL}/__adminhidden/`);
  console.log(`Local dev URL:   http://localhost:${VITE_PORT}/__adminhidden/`);
  console.log(`Local API URL:   http://localhost:${API_PORT}`);
  console.log("Starting admin API worker...");

  const api = spawnApiWorker(API_PORT);
  console.log("Starting vite dev...");
  const vite = spawnViteDev(VITE_PORT, API_PORT);
  console.log("Starting Cloudflare Tunnel admin-local...");
