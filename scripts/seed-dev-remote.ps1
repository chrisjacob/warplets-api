# seed-dev-remote.ps1
# Applies the 10K warplets_metadata seed to the REMOTE dev D1 database
# in chunks to avoid payload/timeout limits.
#
# Usage:
#   powershell -ExecutionPolicy Bypass -File .\scripts\seed-dev-remote.ps1
# Optional:
#   -ChunkSize <rows_per_batch>  (default: 200)
#   -StartToken <token_id>       (default: 1)
...
$tmpDir = Join-Path $env:TEMP "warplets-seed-dev-remote-chunks"
...
for ($i = 0; $i -lt $total; $i += $ChunkSize) {
    ...
    $result = pnpm exec wrangler d1 execute WARPLETS --env dev --remote --file $chunkFile 2>&1
...
Write-Host "Verifying remote dev row count..."
pnpm exec wrangler d1 execute WARPLETS --env dev --remote --command "SELECT COUNT(*) AS total_rows, MIN(token_id) AS min_token, MAX(token_id) AS max_token FROM warplets_metadata;"
