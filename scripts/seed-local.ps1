# seed-local.ps1
# Applies the 10K warplets_metadata seed to the LOCAL app D1 database
# in small chunks to avoid payload/timeout limits.
#
# Usage:
#   powershell -ExecutionPolicy Bypass -File .\scripts\seed-local.ps1
# Optional:
#   -ChunkSize <rows_per_batch>  (default: 400)
...
$tmpDir = Join-Path $env:TEMP "warplets-seed-local-chunks"
...
for ($i = 0; $i -lt $total; $i += $ChunkSize) {
    ...
    try {
        $result = pnpm exec wrangler d1 execute WARPLETS --local --file $chunkFile 2>&1
    } finally {
        Pop-Location
    }
...
Write-Host "Verifying local row count..."

Push-Location $appDir
try {
    pnpm exec wrangler d1 execute WARPLETS --local --command "SELECT COUNT(*) AS total_rows, MIN(token_id) AS min_token, MAX(token_id) AS max_token FROM warplets_metadata;"
} finally {
    Pop-Location
}
