...
const PUBLIC_URL = "https://million-local.10x.meme";
...
function spawnCloudflared(port) {
  ...
  const command = process.platform === "win32"
    ? `"${executable}" tunnel run --url http://127.0.0.1:${port} million-local`
    : executable;
  const args = process.platform === "win32"
    ? []
    : ["tunnel", "run", "--url", `http://127.0.0.1:${port}`, "million-local"];
...
async function waitForApi(port) {
  ...
  while (Date.now() < deadline) {
    try {
      const res = await fetch(`http://127.0.0.1:${port}/api/million-status?entries=1`);
